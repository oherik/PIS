 ...
 */
QueuePtr new_queue() {
    QueuePtr head =  malloc(sizeof (struct QueueElement));   // Allocates memory for the head
    head->prio = MAX_PRIO;
    return head;                      
}
...
void add(QueuePtr q, int prio, DataPtr d) {
    QueuePtr new_element = malloc(sizeof (struct QueueElement));  // Allocates memory for the new element
